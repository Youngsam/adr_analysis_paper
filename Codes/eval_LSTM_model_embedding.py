...
# Using pre-trained sentence embedding
pretrained_embed = pickle.load(open('./data_prep/pretrained_sents_embed.pkl', 'rb'))
pretrained_embed = np.vstack((np.zeros(200, dtype=np.float32), pretrained_embed))
embed_size = 200

with tf.name_scope("Embeddings"):
    embedding = tf.constant(pretrained_embed, name='pretrained')
...
with tf.Session() as sess:
    ...
    for e in range(epochs):
        ...
        for ii, (x, y) in enumerate(get_batches(train_x, train_y, batch_size), 1):
            ...
            saver.save(sess, "checkpoints_embed2/sentiment_manish.ckpt")
    saver.save(sess, "checkpoints_embed2/sentiment_manish.ckpt")

test_acc = []
with tf.Session() as sess:
    saver.restore(sess, "checkpoints_embed2/sentiment_manish.ckpt")
...
for rs in test_acc:
    for r in rs:
        if r[0] == True:
            ...
            c = 0
        mytemp.append(c)
print("accuracy:", np.mean(mytemp))  # 0.5745192307692307
